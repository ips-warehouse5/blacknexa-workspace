...
Two surfaces:
  * `/internal/*` — the news engine and content moderation, called by the Node
    backend with a service token.
...
from app.api.v1.internal import moderation, news

api_router = APIRouter(prefix="/api/v1")

api_router.include_router(news.router, prefix="/internal")
api_router.include_router(moderation.router, prefix="/internal")
